...
fargsitem	: IDENT
	  	{
			  ...
			  strncpy( sp->sy_name, $1->lx_u.lxu_name, NAMMAX);
			  sp->sy_flags = SF_ISFARG;
			  lfree($1);
			  $$ = sp;
		}
;
formalargs	: /* empty */ { $$ = NULL; }
	  	| fargsitem
	 		{
				$$ = $1;
			}
	 	| formalargs ',' fargsitem
	 		{
			  struct sym *sp;
			  
			  for (sp=$1; sp->sy_link!=NULL; sp=sp->sy_link)
			    if( strncmp(sp->sy_name, $3->sy_name, NAMMAX) == 0 ) // f(x,y,x) -- BAD
				panic("duplicate formal argument");
			  if( strncmp(sp->sy_name, $3->sy_name, NAMMAX) == 0 )
			    panic("duplicate formal argument");
